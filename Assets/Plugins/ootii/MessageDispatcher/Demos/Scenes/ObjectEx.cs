...
namespace com.ootii.Demos.MD
{
    public class ObjectEx : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            MessageDispatcher.AddListener(this,"EVERYONE", OnEveryoneMessageReceived);
            /*
            // Same message handler function used, but filter is used to limit who gets it
            if (gameObject.tag == "Sphere") { MessageDispatcher.AddListener(this,"FILTER", "Sphere", OnFilterMessageReceived); }
            if (gameObject.tag == "Cube") { MessageDispatcher.AddListener(this,"FILTER", "Cube", OnFilterMessageReceived); }
            //if (gameObject.tag == "Cylinder") { MessageDispatcher.AddListener(this,"FILTER", "Cylinder", OnFilterMessageReceived); }

            // Objects listening for messages sent to them
            if (gameObject.name == "Sphere_1") { MessageDispatcher.AddListener(this,gameObject, "TARGET", OnTargetedMessageReceived); }
            if (gameObject.name == "Cube_1") { MessageDispatcher.AddListener(this,gameObject, "TARGET", OnTargetedMessageReceived); }
            if (gameObject.name == "Cylinder_1") { MessageDispatcher.AddListener(this,gameObject, "TARGET", OnTargetedMessageReceived); }
    ...
    */
        ...
        void OnMouseDown()
        {
            ...
            {
                //MessageDispatcher.RemoveListener(this,"EVERYONE", OnEveryoneMessageReceived);
               // MessageDispatcher.RemoveListener(this, gameObject, "TARGET", OnTargetedMessageReceived);
